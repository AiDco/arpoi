...
//        sceneLocationView.debugOptions = false
...
//        nodes.append(theAlamo)
        ...
        let pikesPeakLayer = LCTextLayer()
        pikesPeakLayer.frame = CGRect(x: 0, y: 0, width: 200, height: 64)
        pikesPeakLayer.cornerRadius = 32
        pikesPeakLayer.fontSize = 30
        pikesPeakLayer.alignmentMode = .center
        pikesPeakLayer.foregroundColor = UIColor.white.cgColor
        pikesPeakLayer.backgroundColor = UIColor.systemBlue.cgColor

        let pikesPeakLayer1 = LCTextLayer()
        pikesPeakLayer1.frame = CGRect(x: 0, y: 0, width: 200, height: 64)
        pikesPeakLayer1.cornerRadius = 32
        pikesPeakLayer1.fontSize = 30
        pikesPeakLayer1.alignmentMode = .center
        pikesPeakLayer1.foregroundColor = UIColor.white.cgColor
        pikesPeakLayer1.backgroundColor = UIColor.systemTeal.cgColor

        let pikesPeakLayer2 = LCTextLayer()
        pikesPeakLayer2.frame = CGRect(x: 0, y: 0, width: 200, height: 64)
        pikesPeakLayer2.cornerRadius = 32
        pikesPeakLayer2.fontSize = 30
        pikesPeakLayer2.alignmentMode = .center
        pikesPeakLayer2.foregroundColor = UIColor.white.cgColor
        pikesPeakLayer2.backgroundColor = UIColor.systemPurple.cgColor



//        label2.font = UIFont.systemFont(ofSize: 14, weight: .medium)
//        label2.position = .init(x: 100, y: 40)
//        label2.foregroundColor = UIColor.black.cgColor
//        testLayer.addSublayer(label2)
...
//
        _ = Timer.scheduledTimer(withTimeInterval: 1, repeats: true) { _ in
            let myLocation = self.sceneLocationView.sceneLocationManager.currentLocation
//            let myLocation: CLLocation? = CLLocation(latitude: 50.006020, longitude: 36.242156)
            let targetLocation = CLLocation(latitude: 50.400961177427554, longitude: 30.635077327962122)
            let targetLocation1 = CLLocation(latitude: 50.40191854116244, longitude: 30.634228254651497)
            let targetLocation2 = CLLocation(latitude: 50.4017769762898, longitude: 30.636254896093362)
            

            let distance = myLocation!.distance(from: targetLocation)
            let distance1 = myLocation!.distance(from: targetLocation1)
            let distance2 = myLocation!.distance(from: targetLocation2)
//            pikesPeakLayer.string = "\(distance) meters"
            pikesPeakLayer.string = "\(distance.string(with: 1)) meters"
            pikesPeakLayer1.string = "\(distance1.string(with: 1)) meters"
            pikesPeakLayer2.string = "\(distance2.string(with: 1)) meters"
        }

        let test = buildLayerNode(latitude: 50.400961177427554,
                                  longitude: 30.635077327962122,
                                  altitude: 180,
                                  layer: pikesPeakLayer)

        let test1 = buildLayerNode(latitude: 50.40191854116244,
                                  longitude: 30.634228254651497,
                                  altitude: 150,
                                  layer: pikesPeakLayer1)

        let test2 = buildLayerNode(latitude: 50.4017769762898,
                                  longitude: 30.636254896093362,
                                  altitude: 100,
                                  layer: pikesPeakLayer2)
        nodes.append(test)
        nodes.append(test1)
        nodes.append(test2)
...
//        nodes.append(pikesPeak)
    ...
    func updateInfoLabel() {
        ...
        if let myLocation = self.sceneLocationView.sceneLocationManager.currentLocation {
            infoLabel.text!.append("\n Lat: \(myLocation.coordinate.latitude), lon: \(myLocation.coordinate.longitude)")
        }
    }
...
extension Double {
    func string(with digits: Int) -> String {
        return truncatingRemainder(dividingBy: 1) == 0 ? String(format: "%.0f", self) : String(format: "%.\(digits)f", self)
    }
}
